...
function onSocialComplete(e) {
	if (!_.isFunction(globalCallback))
		return;
...
exports.facebook = function(args) {
	...
	// Native iOS dialog
	if (OS_IOS && (dkNappSocial != null && dkNappSocial.isFacebookSupported()) && false === /https?\:\/\/(www\.)?facebook\.com/.test(args.url) // BUG: iOS share dialog doesn't share Facebook links
	) {
...
exports.whatsapp = function(args) {
	...
	if (OS_IOS) {
		Util.openURL('whatsapp://send?text=' + args.fullText, function() {
			require('T/dialog').confirmYes(L('app_not_installed', 'App not installed'), String.format(L('app_install_question', 'You must install %s to proceed.'), 'Whatsapp'), function() {
...
exports.message = exports.sms = function(args) {
	...
	if (OS_ANDROID) {
		var url = 'sms:';
		if (args.recipients != null)
			url += _.isArray(args.recipients) ? args.recipients[0] : args.recipients;
...
exports.activity = function(args) {
	...
	if (OS_ANDROID) {
		...
		if (args.fullText)
			intent.putExtra(Ti.Android.EXTRA_TEXT, args.fullText);
		if (args.title)
			intent.putExtra(Ti.Android.EXTRA_TITLE, args.title);
		if (args.image)
