...
function core_sha1(x, len) {
	x[len >> 5] |= 128 << 24 - len % 32, x[(len + 64 >> 9 << 4) + 15] =
	len;
	var w = Array(80),
	    a = 1732584193,
	    b = -271733879,
...
OAuth == null && ( OAuth = {}), OAuth.setProperties = function(into, from) {
	...
	formEncode : function(parameters) {
		var form = "",
		    list = OAuth.getParameterList(parameters);
	...
	decodeForm : function(form) {
		...
		for (var n = 0; n < nvps.length; ++n) {
			...
			var equals = nvp.indexOf("="),
			    name,
			    value;
			equals < 0 ? ( name = OAuth.decodePercent(nvp),
			value = null) : ( name = OAuth.decodePercent(nvp.substring(0, equals)),
	...
	setParameter : function(message, name, value) {
		...
		if ( parameters instanceof Array) {
			for (var p = 0; p < parameters.length; ++p)
				parameters[p][0] == name && (value === undefined ? parameters.splice(p, 1) : (parameters[p][1] = value,
				value =
				undefined));
			value !== undefined && parameters.push([name, value]);
		} else
			parameters = OAuth.getParameterMap(parameters), parameters[name] =
	...
	makeSubclass : function(getSignatureFunction) {
		...
		return subClass.prototype = new superClass, subClass.prototype.getSignature =
		getSignatureFunction, subClass.prototype.constructor =
		subClass, subClass;
	},
	...
	normalizeUrl : function(url) {
		var uri = OAuth.SignatureMethod.parseUri(url),
		    scheme = uri.protocol.toLowerCase(),
		    authority = uri.authority.toLowerCase(),
		    dropPort = scheme == "http" && uri.port == 80 || scheme == "https" && uri.port == 443;
		if (dropPort) {
...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	function showLoading() {
		if (loading)
			return;
		loading = !0, loadingView.value = 0,
		estimateID = firstLoad ? "tokenRequest" : "pageLoad", estimates[estimateID] || (estimates[estimateID] = firstLoad ? 2000 : 1000),
		firstLoad = !1,
		startTime = (new Date).getTime(),
		intervalID = setInterval(updateProgress, 30), webView && webView.hide(), loadingView && loadingView.show(), loadingContainer && loadingContainer.show();
	}
	...
	function authorizeUICallback(e) {
		if (e.url.indexOf('linkedin') !== -1) {
			...
			response ? ( pin = response, destroyAuthorizeUI(e.cb, true), receivePinCallback()) : (loadingView && loadingView.hide(), loadingContainer && loadingContainer.hide(), webView && webView.show()),
			...
			if ((Titanium.Platform.osname == 'android') && Ti.Platform.version >= '4.4.2') {
				...
				response ? ( pin = response.split("<code>")[1].split("</code>")[0], destroyAuthorizeUI(e.cb, true), receivePinCallback()) : (loadingView && loadingView.hide(), loadingContainer && loadingContainer.hide(), webView && webView.show()),
				loading = !1, clearInterval(intervalID), estimates[estimateID] = (new Date).getTime() - startTime, Ti.App.Properties.setString("Social-LoadingEstimates", JSON.stringify(estimates));
			}
		}
	}
	...
	this.getPin = function() {
		...
		var done = !1,
			...
			onload : function() {
				...
				requestTokenSecret = responseParams.oauth_token_secret, callback({
					success : !0,
	...
	var destroyAuthorizeUI = function(cb, bool) {
		if (window == null)
			return;
		try {
			webView.removeEventListener("load", authorizeUICallback), webView.removeEventListener("beforeload", showLoading), loadingView.hide(), window.close(),
			loading = null,
			webView = null,
			loadingView = null,
			loading = !1,
	    ...
	    firstLoad = !0,
	    loading = !1,
	    estimates = JSON.parse(Ti.App.Properties.getString("Social-LoadingEstimates", "{}")),
	    estimateID,
	    startTime,
	    intervalID = 0;
	this.closeWebView = function(cb) {
		...
		var parameterMap = OAuth.getParameterMap(message.parameters),
			...
			onload : function() {
				...
				accessTokenSecret = responseParams.oauth_token_secret, callback({
					success : !0
				});
			},
			onerror : function() {
...
exports.create = function(settings) {
	var site = (settings.site || "twitter").toLowerCase(),
	    adapter = new OAuthAdapter(settings.consumerSecret, settings.consumerKey, "HMAC-SHA1");
